function init_plot(element_id,data,count_function,color_function,title_function,legend_function,max_level)
{
    ...
    if (legend_function != undefined)
    {
        ...
        function remove_legend(d){ legend.transition().duration(1000).style("opacity","0"); }
    }
    ...
    function animate(d) {
        ...
        if (revert){
            ...
            .filter(
                function (b)
                {
                    if (b[0] >= last_ref[0] && b[1] <= last_ref[1]+0.001  && b[3] >= last_ref[3])
                    {
                        return true;
                    }
                    return false;
            ...
            .filter(
                function (b)
                {
                    if (b[0] < d[0] || b[1] > d[1]-0.001 || b[3] < d[3])
                    {
                        return true;
                    }
                    return false;
        ...
        .filter(
            function (b){
                if (b[0] >= new_ref[0] && b[1] <= new_ref[1]+0.001 && b[3] >= new_ref[3]){ return true; }
                return false;
            }
        )
